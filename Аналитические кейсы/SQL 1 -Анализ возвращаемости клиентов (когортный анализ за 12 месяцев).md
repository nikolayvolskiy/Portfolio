...
WITH customer_first_purchase AS (
    SELECT 
        customer_phone,
        MIN(DATE(sales_time)) as first_purchase_date,
        strftime('%Y-%m', MIN(sales_time)) as cohort_month
    FROM main_sales
    WHERE customer_phone IS NOT NULL 
      AND customer_phone != ''
      AND DATE(sales_time) >= DATE('now', '-12 months')
...
cohort_analysis AS (
    SELECT 
        cp.cohort_month,
        cp.customer_phone,
        (strftime('%Y', ms.sales_time) * 12 + strftime('%m', ms.sales_time)) - 
        (strftime('%Y', cp.first_purchase_date) * 12 + strftime('%m', cp.first_purchase_date)) as months_since_first
    FROM customer_first_purchase cp
    LEFT JOIN main_sales ms ON cp.customer_phone = ms.customer_phone
      AND DATE(ms.sales_time) >= cp.first_purchase_date
      AND DATE(ms.sales_time) < DATE(cp.first_purchase_date, '+12 months')
